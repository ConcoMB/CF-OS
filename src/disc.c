...
int _disk_read(int ata, char * ans, int numreads, unsigned int sector){
   ...
   while(retry) {
      ...
      _IO_out(0x1F1, 0x00);
      _IO_out(0x1F2, 0x00); 
      _IO_out(0x1F2, numreads);
      _IO_out(0x1F3, (unsigned char)(addr >> 24));
      _IO_out(0x1F3, (unsigned char)addr);
      _IO_out(0x1F4, 0);
      _IO_out(0x1F4, (unsigned char)(addr >> 8));
      _IO_out(0x1F5, 0);
      _IO_out(0x1F5, (unsigned char)(addr >> 16));
      _IO_out(0x1F6, 0);
      _IO_out(0x1F7, 0x24);
...
int _disk_write(int ata, char * msg, int numreads, unsigned int sector){
   ...
   _IO_out(0x1F3, (unsigned char)(addr >> 24));
   _IO_out(0x1F3, (unsigned char)addr);
   _IO_out(0x1F4, (unsigned char)(addr >> 32));
   _IO_out(0x1F4, (unsigned char)(addr >> 8));
   _IO_out(0x1F5, (unsigned char)(addr >> 40));
   _IO_out(0x1F5, (unsigned char)(addr >> 16));
   _IO_out(0x1F6, 0);
   _IO_out(0x1F7, 0x34);
...
void writeDataToRegister (int ata, char upper, char lower) {
   ...
   _IO_out_w(ata + WIN_REG0, out);   
}

//converts data from ATA language
unsigned short getDataRegister (int ata) {
   unsigned short ans;
   ans = _IO_in_w(ata + WIN_REG0);
   return ans;
}

void identifyDevice (int ata) {
   _IO_out(ata + WIN_REG6, 0xA0);
   _IO_out(ata + WIN_REG7, 0xEC);
}
